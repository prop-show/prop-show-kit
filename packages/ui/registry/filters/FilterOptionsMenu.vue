...
const RenderNode: FunctionalComponent<{ content?: VNodeChild }> = (renderProps) =>
  renderProps.content;
...
const effectiveValues = computed(() =>
  props.controlled && props.field.value !== undefined ? props.field.value : props.values,
);
...
const selectedOptions = computed(() => {
  ...
  if (!searchInput.value.trim()) return selected;
  if (!isAsync.value)
...
const unselectedOptions = computed(() => {
  ...
  return options.filter((option) => {
    if (effectiveValues.value.includes(option.value)) return false;
...
function focusActiveControl() {
  nextTick(() => {
    if (props.field.searchable === false) listboxRef.value?.focus();
    else (inputRef.value?.$el as HTMLInputElement | undefined)?.focus();
  });
}

function commitValues(values: T[]) {
  if (props.controlled && props.field.onValueChange) props.field.onValueChange(values);
  else emit("update:values", values);
}
...
function toggleOption(option: FilterOption<T>) {
  ...
  commitValues(values);
  if (!isMultiSelect.value) emit("close");
}

function preventMultiSelectClose(event: Event) {
  if (isMultiSelect.value) event.preventDefault();
}
  ...
  if (event.key === "ArrowDown") {
    ...
    if (allOptions.value.length > 0) {
      highlightedIndex.value =
        highlightedIndex.value < allOptions.value.length - 1 ? highlightedIndex.value + 1 : 0;
    }
  } else if (event.key === "ArrowUp") {
    event.preventDefault();
    if (allOptions.value.length > 0) {
      highlightedIndex.value =
        highlightedIndex.value > 0 ? highlightedIndex.value - 1 : allOptions.value.length - 1;
    }
...
watch(
  ...
  ([active, length]) => {
    if (active && length > 0 && highlightedIndex.value < 0) highlightedIndex.value = 0;
  },
  { immediate: true },
);
        ...
        >
          ...
          <DropdownMenuGroup v-if="unselectedOptions.length > 0" class="px-1">
            <DropdownMenuCheckboxItem
              ...
              :data-highlighted="
                highlightedIndex === index + selectedOptions.length ? '' : undefined
              "
              :class="
                cn(
                  'data-highlighted:bg-accent data-highlighted:text-accent-foreground',
