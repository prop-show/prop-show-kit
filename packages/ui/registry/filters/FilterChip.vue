...
import { getOperatorsForField, resolveSelectedOptions, useFilterContext } from "./filter-core";
...
const selectedOptions = computed(() => resolveSelectedOptions(props.field, effectiveValues.value));
...
function updateValues(values: T[]) {
  if (props.field.onValueChange) props.field.onValueChange(values);
  else emit("update", { values });
}
  ...
  <ButtonGroup>
    ...
    <template v-if="filter.operator !== 'empty' && filter.operator !== 'not_empty'">
      ...
      <InputGroup
        ...
        ref="inputGroup"
        :class="
          cn('w-36', context.size === 'sm' && 'h-7!', context.size === 'lg' && 'h-9!', field.class)
        "
      >
        ...
        <InputGroupInput
          ...
          "
          :class="cn(context.size === 'sm' && 'h-7! text-xs', context.size === 'lg' && 'h-9!')"
      ...
      <DropdownMenu v-else v-model:open="optionsOpen">
        <DropdownMenuTrigger as-child>
          <Button variant="outline" :size="context.size">
            ...
            <template v-else>
              <span v-if="selectedOptions.length > 0" class="flex items-center gap-0">
                <template v-for="option in selectedOptions.slice(0, 3)" :key="String(option.value)">
                  <component :is="option.icon" v-if="option.icon" class="-ms-1.5 first:ms-0" />
                </template>
              </span>
              {{
