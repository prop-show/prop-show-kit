...
const selectableFields = computed(() => flattenFields(props.fields).filter((field) => {
...
const filteredFields = computed(() => {
  ...
  return selectableFields.value.filter(field =>
    !query || field.label?.toLowerCase().includes(query),
  )
})
...
function handleSubmenuOpenChange(field: FilterFieldConfig<T>, open: boolean) {
  ...
  else if (openSubMenu.value === field.key)
    closeFieldSubmenu()
}
  ...
  if (event.key === 'ArrowDown') {
    ...
    if (filteredFields.value.length > 0) {
      highlightedIndex.value
        = highlightedIndex.value < filteredFields.value.length - 1
    ...
    if (field?.key) {
      if (fieldHasSubmenu(field))
        openSubMenu.value === field.key ? closeFieldSubmenu() : openFieldSubmenu(field)
      else
        addFilter(field.key)
    }
  }
...
<template>
  <div :class="cn(filtersContainerVariants({ variant, size }), props.class)">
    <DropdownMenu v-if="selectableFields.length > 0" v-model:open="addFilterOpen">
      ...
      <DropdownMenuContent
        align="start"
        :class="cn('w-[220px]', menuPopupClass)"
      >
                ...
                <template v-for="(field, index) in filteredFields" :key="field.key">
                  ...
                  >
                    <DropdownMenuSubTrigger
                      ...
                      @mouseenter="highlightedIndex = index; activeMenu = 'root'"
                    >
