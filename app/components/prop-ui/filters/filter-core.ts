...
export function getOperatorsForField<T>(
  ...
  const fieldType = field.type === 'multiselect' || (field.type === 'select' && values.length > 1)
    ? 'multiselect'
    : (field.type ?? 'select')
...
export function useFieldOptions<T>(
  ...
  watch(
    ...
    async ([loader, isEnabled, query], _previous, onCleanup) => {
      ...
      try {
        ...
        if (cancelled || activeRequest !== requestId)
          return
  ...
  return {
    isAsync,
    options: computed(() => isAsync.value ? asyncOptions.value : (currentField.value.options ?? [])),
    loading,
    error,
