...
const { data: currentDoc } = await useAsyncData(route.path, () => {
    ...
    const path = `/${locale.value}/docs/${slugStr}`
...
<template>
    <div v-if="currentDoc" class="flex flex-col lg:flex-row gap-6 lg:gap-10 relative items-start max-w-screen md:max-w-full ">
        <div class="prose dark:prose-invert prose-pre:bg-black flex-1 min-w-0 max-w-none">
